...
pub(crate) struct CommanderLocalState<As: AllSignalTypes, Mpsc: MultiProducerSingleConsumer> {
    sender_to_commander: Mutex<Mpsc::Sender<ui_model::Message>>,
    sender_to_process_manager:
        Mutex<Mpsc::Sender<process_manager::MessageToProcessManager<As, Mpsc>>>,
    user_uuid: Mutex<Option<db_types::UuidType>>,
    selected_company_branch: Mutex<Option<db_types::UuidType>>,
    aborter_to_company_and_branch_listener: Mutex<Option<Box<dyn FnOnce()>>>,
}
...
pub(crate) trait MVU {
    ...
    >(
        ...
        model: ui_model::Model<As>,
        cache: cache_actor::Cache<At, Mpsc>,
        commander_local_state: Arc<CommanderLocalState<As, Mpsc>>,
    );
}
                    ...
                    Some(_) => {
                        commander_local_state
                            ...
                            .send(ui_model::Message::CompanyAndBranchSelection(
                                ui_effect::company_and_branch_selection::Msg::Subscribe,
                            ))
                            .await
                            .unwrap();
                    }
...
pub mod create_company {
    ...
    fn handel_close<As: AllSignalTypes>(model: ui_model::Model<As>) {
        let page_create_company = model
            .page_root
            .page_after_auth
            .page_company_branch_selection
